...
exports.fluentHtmlExtractor = fluentHtmlExtractor;
/**
 ...
 */
function stripTypeCasts(content) {
...
function arbitraryOrStandard(prefix, args) {
    if (args.length === 1)
...
function spacingClass(prefix, args) {
    if (args.length === 1)
        return [`${prefix}-${args[0]}`];
    if (args.length === 2) {
        if (UNITS.has(args[0])) {
...
const METHOD_PATTERNS = [
    ...
    },
    {
        ...
        generateClass: (args) => {
            if (args.length === 1)
                return [`overscroll-${args[0]}`];
            if (args.length === 2)
                return [`overscroll-${args[0]}-${args[1]}`];
            return [];
        },
    },
    // --- List Style ---
 ...
 */
function extractDirectClasses(content) {
    const classes = [];
 ...
 */
function extractVariantClasses(content, outerPrefix = "") {
    const classes = [];
 ...
 */
function extractDefaultClasses(content) {
 ...
 */
function fluentHtmlExtractor(content, options) {
